...
class Trading():
    ...
    def action(self, symbol):
        ...
        if self.option.prints and self.order_id == 0:
            ...
            print('-> %s -price:%.8f buyp:%.8f sellp:%.8f-bid:%.8f ask:%.8f spread:%.2f' % (time.strftime('%l:%M%p %Z on %b %d, %Y'), lastPrice, buyPrice, profitableSellingPrice, lastBid, lastAsk, spreadPerc))
